...
public class Main {
    ...
    public static void usunSpotkanie() {
        int dzien = podajDzien();
        List<Spotkanie> spotkania = kalendarz.ZwrocSpotkaniaZDnia(dzien);
        if (spotkania.isEmpty()) {
            ...
            while (true) {
                ...
                for (int i = 0; i < spotkania.size(); i++) {
                    ...
                    System.out.println(i+1 + "." + " Opis: " + spotkanie.getOpis() + ", czas rozpoczęcia: " + spotkanie.getCzasPoczatku() + ", czas zakończenia: " + spotkanie.getCzasZakonczenia() + ", priorytet " + spotkanie.getPriorytet());
                }
    ...
    public static void wyswietlSpotkaniaDniaPriorytet() {
        int dzien = podajDzien();
        String priorytet = podajPriorytet();
        List<Spotkanie> spotkania = kalendarz.ZwrocSpotkaniaZDniaPriorytet(dzien, priorytet);
        if (spotkania.isEmpty()) {
            System.out.println("Nie ma żadnych spotkań tego dnia!");
        } else {
            for (int i = 0; i < spotkania.size(); i++) {
    ...
    public static void wyswietlSpotkaniaDniaOdGodziny() {
        ...
        List<Spotkanie> spotkania = kalendarz.ZwrocSpotaniaZDniaOdGodziny(dzien, godzina);
        if (spotkania.isEmpty()) {
            System.out.println("Nie ma żadnych spotkań tego dnia!");
        } else {
            for (int i = 0; i < spotkania.size(); i++) {
    ...
    private static void displayMenu() {
        ...
        System.out.println("5. Wyświetl spotkania zaczynające się po podanej godzinie");
        System.out.println("6. Funkcja testowa dodjąca 7 spotkań na poniedziałek");
        System.out.println("7. Wyjść z programu");
